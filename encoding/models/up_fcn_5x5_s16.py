...
__all__ = ['up_fcn_5x5_s16', 'get_up_fcn_5x5_s16']

class up_fcn_5x5_s16(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(up_fcn_5x5_s16, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = up_fcn_5x5_s16Head(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
class up_fcn_5x5_s16Head(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(up_fcn_5x5_s16Head, self).__init__()
...
class localUp(nn.Module):
    ...
    def forward(self, c1,c2,out):
        ...
        unfold_up_c2 = unfold(c2, 5, 1, 2, 1).permute(0,2,1).view(n, h*w, -1, 5*5)
        # torch.nn.functional.unfold(input, kernel_size, dilation=1, padding=0, stride=1)
        energy = torch.matmul(c1.view(n, -1, h*w).permute(0,2,1).unsqueeze(2), unfold_up_c2).squeeze(2) #n,h*w,k*k
        att = torch.softmax(energy, dim=-1)
        out = interpolate(out, (h,w), **self._up_kwargs)
        unfold_out = unfold(out, 5, 1, 2, 1).permute(0,2,1).view(n, h*w, -1, 5*5)
        out = torch.matmul(unfold_out, att.unsqueeze(3)).squeeze(3).permute(0,2,1).view(n,-1,h,w)

        return out


def get_up_fcn_5x5_s16(dataset='pascal_voc', backbone='resnet50', pretrained=False,
            root='~/.encoding/models', **kwargs):
    r"""up_fcn_5x5_s16 model from the paper `"Fully Convolutional Network for semantic segmentation"
    <https://people.eecs.berkeley.edu/~jonlong/long_shelhamer_up_fcn_5x5_s16.pdf>`_
    Parameters
    ----------
    ...
    Examples
    --------
    >>> model = get_up_fcn_5x5_s16(dataset='pascal_voc', backbone='resnet50', pretrained=False)
    >>> print(model)
    """
    acronyms = {
        'pascal_voc': 'voc',
    ...
    model = up_fcn_5x5_s16(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
        from .model_store import get_model_file
        model.load_state_dict(torch.load(
            get_model_file('up_fcn_5x5_s16_%s_%s'%(backbone, acronyms[dataset]), root=root)))
    return model
