...
__all__ = ['up_fcn_dilation_s8plus', 'get_up_fcn_dilation_s8plus']

class up_fcn_dilation_s8plus(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(up_fcn_dilation_s8plus, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = up_fcn_dilation_s8plusHead(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
class up_fcn_dilation_s8plusHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(up_fcn_dilation_s8plusHead, self).__init__()
...
class localUp(nn.Module):
    ...
    def forward(self, c1,c2,out):
        ...
        unfold_up_c2 = unfold(c2, 3, 2, 2, 1).permute(0,2,1).view(n, h*w, -1, 3*3)
...
def get_up_fcn_dilation_s8plus(dataset='pascal_voc', backbone='resnet50', pretrained=False,
            root='~/.encoding/models', **kwargs):
    r"""up_fcn_dilation_s8plus model from the paper `"Fully Convolutional Network for semantic segmentation"
    <https://people.eecs.berkeley.edu/~jonlong/long_shelhamer_up_fcn_dilation_s8plus.pdf>`_
    Parameters
    ----------
    ...
    Examples
    --------
    >>> model = get_up_fcn_dilation_s8plus(dataset='pascal_voc', backbone='resnet50', pretrained=False)
    >>> print(model)
    """
    acronyms = {
        'pascal_voc': 'voc',
    ...
    model = up_fcn_dilation_s8plus(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
        from .model_store import get_model_file
        model.load_state_dict(torch.load(
            get_model_file('up_fcn_dilation_s8plus_%s_%s'%(backbone, acronyms[dataset]), root=root)))
    return model
