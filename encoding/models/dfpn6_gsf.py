...
class dfpn6_gsfHead(nn.Module):
    ...
    def forward(self, c1,c2,c3,c4,c20,c30,c40):
        ...
        out3 = self.project3(p3)+self.project4_2(F.interpolate(out4, (h3,w3), **self._up_kwargs))
        out2 = self.localUp3(c2, out3)
        p2_1 = self.dconv2_1(out2)
        p2_8 = self.dconv2_8(out2)
        p2 = torch.cat([p2_1,p2_8], 1)
        out = self.project2(p2)+self.project3_2(F.interpolate(out3, (h,w), **self._up_kwargs))
