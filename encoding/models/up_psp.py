...
class PyramidPooling(nn.Module):
    ...
    def forward(self, x):
        _, _, h, w = x.size()
        feat1 = interpolate(self.conv1(self.pool1(x)), (h, w), **self._up_kwargs)
        feat2 = interpolate(self.conv2(self.pool2(x)), (h, w), **self._up_kwargs)
        feat3 = interpolate(self.conv3(self.pool3(x)), (h, w), **self._up_kwargs)
        feat4 = interpolate(self.conv4(self.pool4(x)), (h, w), **self._up_kwargs)
