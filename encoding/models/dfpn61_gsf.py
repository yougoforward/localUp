...
class dfpn61_gsfHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
        ...
        self.project3_2 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, padding=1, dilation=1, bias=False),
                                   norm_layer(inter_channels),
                                   nn.ReLU())
        ...
        self.project2_2 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, padding=1, dilation=1, bias=False),
                                   norm_layer(inter_channels),
                                   nn.ReLU(),
                                   )
    def forward(self, c1,c2,c3,c4,c20,c30,c40):
        ...
        p3 = self.project3_2(p4+p3)

        p3 = F.interpolate(p3, (h,w), **self._up_kwargs)
        out = self.project2_2(p3+p2)
        #gp
        gp = self.gap(c4)        
        # se
        se = self.se(gp)
