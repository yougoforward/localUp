...
__all__ = ['fcn_fpn', 'get_fcn_fpn']

class fcn_fpn(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(fcn_fpn, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = fcn_fpnHead(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
def get_fcn_fpn(dataset='pascal_voc', backbone='resnet50', pretrained=False,
            root='~/.encoding/models', **kwargs):
    r"""fcn_fpn model from the paper `"Fully Convolutional Network for semantic segmentation"
    <https://people.eecs.berkeley.edu/~jonlong/long_shelhamer_fcn_fpn.pdf>`_
    Parameters
    ----------
    ...
    Examples
    --------
    >>> model = get_fcn_fpn(dataset='pascal_voc', backbone='resnet50', pretrained=False)
    >>> print(model)
    """
    acronyms = {
        'pascal_voc': 'voc',
    ...
    model = fcn_fpn(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
        from .model_store import get_model_file
        model.load_state_dict(torch.load(
            get_model_file('fcn_fpn_%s_%s'%(backbone, acronyms[dataset]), root=root)))
    return model
