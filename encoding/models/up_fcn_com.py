...
__all__ = ['up_fcn_com', 'get_up_fcn_com']

class up_fcn_com(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(up_fcn_com, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = up_fcn_comHead(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
class up_fcn_comHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(up_fcn_comHead, self).__init__()
    ...
    def forward(self, c1,c2,c3,c4,c20,c30,c40):
        n,c,h,w =c1.size()
        out1 = self.conv5(c4)
        out2 = self.localUp4(c3, c40, out1)
        out3 = self.localUp3(c2, c30, out2)
        out4 = self.localUp2(c1, c20, out3)
        out1 = interpolate(out1, (h,w), **self._up_kwargs)
        out2 = interpolate(out2, (h,w), **self._up_kwargs)
        out3 = interpolate(out3, (h,w), **self._up_kwargs)
        out = sum([out1,out2,out3,out4])
...
def get_up_fcn_com(dataset='pascal_voc', backbone='resnet50', pretrained=False,
            root='~/.encoding/models', **kwargs):
    r"""up_fcn_com model from the paper `"Fully Convolutional Network for semantic segmentation"
    <https://people.eecs.berkeley.edu/~jonlong/long_shelhamer_up_fcn_com.pdf>`_
    Parameters
    ----------
    ...
    Examples
    --------
    >>> model = get_up_fcn_com(dataset='pascal_voc', backbone='resnet50', pretrained=False)
    >>> print(model)
    """
    acronyms = {
        'pascal_voc': 'voc',
    ...
    model = up_fcn_com(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
        from .model_store import get_model_file
        model.load_state_dict(torch.load(
            get_model_file('up_fcn_com_%s_%s'%(backbone, acronyms[dataset]), root=root)))
    return model
