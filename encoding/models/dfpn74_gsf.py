...
__all__ = ['dfpn74_gsf', 'get_dfpn74_gsf']


class dfpn74_gsf(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(dfpn74_gsf, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = dfpn74_gsfHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class dfpn74_gsfHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(dfpn74_gsfHead, self).__init__()
...
class localUp(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        ...
        self._up_kwargs = up_kwargs
        self.refine = nn.Sequential(nn.Conv2d(2*out_channels, 256, 1, padding=0, dilation=1, bias=False),
                                   norm_layer(256),
                                   nn.ReLU(),
                                   nn.Conv2d(256, 256, 3, padding=1, dilation=1, bias=False),
                                   norm_layer(256),
                                   nn.ReLU(),
                                   nn.Conv2d(256, out_channels, 1, padding=0, dilation=1, bias=False),
                                   norm_layer(out_channels),
                                    )
        self.relu = nn.ReLU()
        self.residual = nn.Sequential(nn.Conv2d(2*out_channels, out_channels, 1, padding=0, dilation=1, bias=False),
                                   norm_layer(out_channels))
    def forward(self, c1,c2):
        ...
        out = self.refine(cat)
        out = self.relu(self.residual(cat)+out)
        return out
...
def get_dfpn74_gsf(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = dfpn74_gsf(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
