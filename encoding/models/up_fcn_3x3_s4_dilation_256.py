...
__all__ = ['up_fcn_3x3_s4_dilation_256', 'get_up_fcn_3x3_s4_dilation_256']

class up_fcn_3x3_s4_dilation_256(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(up_fcn_3x3_s4_dilation_256, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = up_fcn_3x3_s4_dilation_256Head(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
def get_up_fcn_3x3_s4_dilation_256(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    r"""up_fcn_3x3_s4_dilation_256 model from the paper `"Fully Convolutional Network for semantic segmentation"
    <https://people.eecs.berkeley.edu/~jonlong/long_shelhamer_up_fcn_3x3_s4_dilation_256.pdf>`_
    Parameters
    ----------
    ...
    Examples
    --------
    >>> model = get_up_fcn_3x3_s4_dilation_256(dataset='pascal_voc', backbone='resnet50', pretrained=False)
    >>> print(model)
    """
    acronyms = {
        'pascal_voc': 'voc',
    ...
    model = up_fcn_3x3_s4_dilation_256(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
        from .model_store import get_model_file
        model.load_state_dict(torch.load(
            get_model_file('up_fcn_3x3_s4_dilation_256_%s_%s'%(backbone, acronyms[dataset]), root=root)))
    return model
