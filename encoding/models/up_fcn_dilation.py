...
__all__ = ['up_fcn_dilation', 'get_up_fcn_dilation']

class up_fcn_dilation(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(up_fcn_dilation, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = up_fcn_dilationHead(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
class up_fcn_dilationHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(up_fcn_dilationHead, self).__init__()
...
def get_up_fcn_dilation(dataset='pascal_voc', backbone='resnet50', pretrained=False,
            root='~/.encoding/models', **kwargs):
    r"""up_fcn_dilation model from the paper `"Fully Convolutional Network for semantic segmentation"
    <https://people.eecs.berkeley.edu/~jonlong/long_shelhamer_up_fcn_dilation.pdf>`_
    Parameters
    ----------
    ...
    Examples
    --------
    >>> model = get_up_fcn_dilation(dataset='pascal_voc', backbone='resnet50', pretrained=False)
    >>> print(model)
    """
    acronyms = {
        'pascal_voc': 'voc',
    ...
    model = up_fcn_dilation(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
        from .model_store import get_model_file
        model.load_state_dict(torch.load(
            get_model_file('up_fcn_dilation_%s_%s'%(backbone, acronyms[dataset]), root=root)))
    return model
